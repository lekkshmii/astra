...
class MeanReversionStrategy(BaseStrategy):
    ...
    def __init__(self, window: int = 20, threshold: float = 2.0, **kwargs):
...
class PairsTradingStrategy(BaseStrategy):
    ...
    def __init__(self, pair: tuple, window: int = 30, threshold: float = 2.0, **kwargs):
    ...
    def generate_signals(self, data: pd.DataFrame) -> pd.DataFrame:
        ...
        if len(self.pair) != 2 or not all(symbol in data.columns for symbol in self.pair):
            raise ValueError(f"Both symbols in pair {self.pair} must be in data")
...
class VolatilityBreakoutStrategy(BaseStrategy):
    ...
    def __init__(self, window: int = 20, multiplier: float = 2.0, **kwargs):
